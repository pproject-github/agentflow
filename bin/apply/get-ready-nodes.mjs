...
function main() {
  ...
  const intermediateDir = path.join(workspaceRoot, ".workspace", "agentflow", "runBuild", flowName, uuid, "intermediate");
  ...
  try {
    ...
    for (const id of pendingInstances) {
      ...
      if (nodeDefinitions[id] === "control_if") {
        ...
        if (outEdge && outEdge.target != null) nextPending.add(outEdge.target);
      } else {
