 ...
 * 当参数个数 ≥4 且第 4 个参数存在时，用 args[3] 作为 flowDir（如 getFlowDir 解析出的 .workspace/agentflow/pipelines/<name>）；否则从 .cursor/agentflow/pipelines/<flowName> 读 flow。
...
/** 解析 ${input}/${output} 占位符的实际值；需 workspaceRoot、flowName，可选 uuid；flowData 为 loadFlowDefinition 结果，有则从 instances 读。可选 flowDirIn：传入时作为流程目录（支持 .workspace 下 flow）。 */
function resolvePlaceholders(workspaceRoot, flowName, order, edges, uuid, flowData, flowDirIn) {
  const flowDir = flowDirIn != null && flowDirIn !== "" ? path.resolve(flowDirIn) : path.join(workspaceRoot, ".cursor", "agentflow", "pipelines", flowName);
  ...
  const intermediateDir = uuid && flowName ? path.join(workspaceRoot, ".workspace", "agentflow", "runBuild", flowName, uuid, "intermediate") : null;
  let execIds = {};
...
function readInstanceStatus(workspaceRoot, flowName, uuid, order) {
  const intermediateDir = path.resolve(workspaceRoot, ".workspace", "agentflow", "runBuild", flowName, uuid, "intermediate");
...
function main() {
  ...
  try {
    ...
    if (uuid && workspaceRoot && flowNameArg) {
      const intermediateDir = path.join(workspaceRoot, ".workspace", "agentflow", "runBuild", flowNameArg, uuid, "intermediate");
