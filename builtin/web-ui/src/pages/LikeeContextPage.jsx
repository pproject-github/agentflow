...
function SectionHeader({ kicker, title, children }) {
  return (
    <div className="likee-section-copy">
      <p className="likee-section-kicker">{kicker}</p>
      <h2>{title}</h2>
      <p>{children}</p>
    </div>
  );
...
function SkillsSection() {
  return (
    <section id="likee-skills" className="likee-section likee-section-dark">
      <div className="likee-section-inner likee-two-column likee-two-column-skills">
        <SectionHeader kicker="SKILLS" title="用 Skills 把 Likee 上下文变成可复用能力">
...
export default function LikeeContextPage() {
  return (
    <div className="likee-page">
      <div className="likee-top-actions">
        <a href="/projects" className="likee-home-link">
      ...
      <section className="likee-hero">
        <WorkspaceScene />
        <div className="likee-hero-content">
          <p className="likee-eyebrow">LIKEE CONTEXT WORKSPACE</p>
          <h1>Likee Agent 工作台</h1>
          <p className="likee-hero-subtitle">提供 Likee 全局上下文，赋能需求挖掘、书写和运营。</p>
          <div className="likee-signal-row">
            <span>查业务逻辑</span>
            <span>查 CK / Hive</span>
            <span>生成原型</span>
            <span>生成 PRD</span>
            <span>同步 TAPD / Git</span>
          </div>
          <div className="likee-hero-actions">
            <a href="/projects" className="likee-primary-action">
      ...
      <section id="likee-scenarios" className="likee-section">
        <div className="likee-section-inner">
          <SectionHeader kicker="SCENARIO TRIGGERS" title="打造 Likee 团队全景上下文">
            不要求先会写 Prompt，也不要求知道该找哪个系统。选择一个业务场景，工作台会把需要的 Likee 上下文、数据查询和最终产物组织起来。
          </SectionHeader>
          <div className="likee-scenario-grid">
              ...
              <article className="likee-usecase-card" key={useCase.label}>
                <div className={`likee-usecase-media ${useCase.images.length > 1 ? "likee-usecase-media-split" : ""}`}>
                  {useCase.images.map((image, index) => (
                    <img key={image} src={image} alt={`${useCase.title} 工作区截图 ${index + 1}`} loading="lazy" />
                  ))}
                </div>
      ...
      <section className="likee-section">
        <div className="likee-section-inner likee-two-column likee-capability-layout">
          <SectionHeader kicker="CAPABILITY MAP" title="每个场景背后，都连接同一套 Likee 上下文能力">
